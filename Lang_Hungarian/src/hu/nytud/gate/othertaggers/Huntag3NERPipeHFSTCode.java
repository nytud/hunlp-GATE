...
/**
 * Hungarian NER (Named Entity Recognition) with the huntag3 tagger and the NER-szeged-hfst model.
 */
@CreoleResource(name = "HU 4. \"emNer\" Named Entity Recognizer (HunTag3, hfst, native)", 
...
public class Huntag3NERPipeHFSTCode extends AbstractLanguageAnalyser {
	...
	private void parseSentence(AnnotationSet annotationSet, Annotation sentence, int nsent) throws GateRuntimeException {
        ...
        String placeHolder = "_"; // needed for huntag3 instead of empty strings  
        ...
        try {
        	synchronized (lock) { 
		    	...
		    	if (huntag == null || !huntag.isAlive()) {
		    	    ...
		    	    ProcessBuilder pb = new ProcessBuilder(bin.getCanonicalPath(),"NER_hfst");
              ...
              // 3. pos + 4. feat (<- from hfstana)
              {
                ...
                String tmpPos = parts[0];
                if ( ! tmpPos.equals("OTHER") ) { tmpPos += "]"; }

                String tmpFeat = placeHolder;
                if ( parts.length > 1 ) { tmpFeat = parts[1].equals("") ? placeHolder : parts[1]; }

                input += "\t" + tmpPos + "\t" + tmpFeat;
              }
			        ...
			        try {
				    	for (int t=0; t<60000; ++t) {
				    		...
				    		if (ht_is.ready()) {
					    		...
					    		if (data.length > 4) tok.getFeatures().put(outputAnnotationType, data[4]);
					    		break;
							}
							Thread.sleep(1);
					    }
