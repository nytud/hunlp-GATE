...
public class Stemmer {
	...
	public Stem process(String input) {
		...
		*/
		...
		{					
			...
			case 1:
				if( c == ']' )
				{
					...
					if (morph.flags == null) morph.flags = new HashSet<>();
					...
					if (iItIsStem){
						...
						if (stem.iStemCode==-1) stem.iStemCode = stem.morphs.size()-1;//save pos...
		...
		if (surface != "") { // surface form es nincs utana semmi
			MorphemeInfo last = stem.morphs.size() > 0 ? stem.morphs.get(stem.morphs.size()-1) : null;
		...
		*/
		...
		for(int n=1; n<stem.morphs.size()-1; n++) {
			if (!stem.morphs.get(n-1).isStem || !stem.morphs.get(n+1).isStem) continue;
			MorphemeInfo m = stem.morphs.get(n);
		...
		if (internalPunctAND && iHyphenPos!=-1 && !compound){
			...
			if (stem.morphs.size()>=nLastStemCode) {
				for (int n=0; n<=nLastStemCode; n++) {
					if (!stem.morphs.get(n).isStem) continue;
					if (n<nLastStemCode)
...
//			}
	...
	{
		if (copy2surface.isEmpty()) return out; //nothing to do :)
		...
		for (int i=0; i<out.length(); i++)
		{
			...
			} else if (in.charAt(i) != out.charAt(i)) return out;
		}
