...
WORDCHARS ({LATIN1}|[a-zA-ZáéíóöõúüûÁÉÍÓÖÕÚÜÛ\-_/§0-9¡£¥¦©ª«¬®¯±³µ¶¹º»¼¾¿ÀÂÃÄÅÆÇÈÊËÌÎÏÐÑÒÔØÙÝÞßàâãäåæçèêëìîïðñòôøùýþ]|"&ndash;"|"&mdash;"|"&"[0-9]+";"?)

/* betûkarakterek a fok, százalékjel, és kötõjel kivételével */

LETTERS   ({LATIN1}|[a-zA-ZáéíóöõúüûÁÉÍÓÖÕÚÜÛ\\/§0-9¡£¥¦©ª«¬®¯±³µ¶¹º»¼¾¿ÀÂÃÄÅÆÇÈÊËÌÎÏÐÑÒÔØÙÝÞßàâãäåæçèêëìîïðñòôøùýþ]|"&ndash;"|"&mdash;"|"&"[0-9]+";"?)

/* mint az elõzõ, számok és perjel nélkül */

LETTERS2 ([a-zA-ZáéíóöõúüûÁÉÍÓÖÕÚÜÛ§¡£¥¦©ª«¬®¯±³µ¶¹º»¼¾¿ÀÂÃÄÅÆÇÈÊËÌÎÏÐÑÒÔØÙÝÞßàâãäåæçèêëìîïðñòôøùýþ]|"&ndash;"|"&mdash;"|"&"[0-9]+";"?)
...
<S>([hH][tT][tT][pP]"://""/"*)?([wW][wW][wW]".")?([a-zA-Z0-9][-a-zA-Z0-9]*|[a-zA-Z0-9])?{SDOMAIN}{LETTERS}{2,} {
		...
		for (i = 11; msd[i] == '-'; i--);
		msd[i + 1] = '\0';
		fprintf(yyout, FORMAT_ANAUJ, yytext, "", yytext, msd);
		msd[2] = 'w';		
		fprintf(yyout, FORMAT_ANAV, yytext, msd);
		fprintf(yyout, FORMAT_UJV);
	}
...
<S>([a-zA-Z0-9]+".")+[0-9]+"/"[a-z0-9]+"."? {
		/* d) pontot ls perjelet tartalmazó azonosító */
		open_begin('i', '-');
	}
...
<S>{LETTERS}+"-"({LETTERS2}+"-")*{NONWORDCHAR}  {
	/* kötõjelre végzõdõ szavak */
			...
			msd[1] = 'h';
			msd[2] = '\0';
			fprintf(yyout, FORMAT_UJ, yytext, "", yytext, msd);
			fprintf(yyout, FORMAT_UJV);
}
...
<OPEN_BEGIN,SFX_K,SFX_E,SFX_J,SFX_END>"\n" {
	...
	if ((msd[1] == OTHER_WORD_SIGN) || (msd[0] != 'O')) {
		if (multitoken == NUM_WORD) {
			for (i = 11; msd[i] == '-'; i--);
			msd[i + 1] = '\0';
			fprintf(yyout, FORMAT_UJ, tok, sfx, tok, msd);
			fprintf(yyout, FORMAT_UJV);			
		} else {
    			fprintf(yyout, FORMAT3, tok, sfx);
		}
	} else {
		for (i = 11; msd[i] == '-'; i--);
		msd[i + 1] = '\0';
		if (multitoken != NORMAL) {
			fprintf(yyout, FORMAT_ANAUJ, tok, sfx, tok, msd);
			print_ana();		    
		} else {
			fprintf(yyout, FORMAT_UJ, tok, sfx, tok, msd);
				}
...
<SFX_END,SFX_NOT>{WORDCHARS}+ {
	...
	if (multitoken == NUM_WORD) {
	    ...
	    if (tok[0] == '\0') {
			msd[1] = 'h';
			msd[2] = '\0';
			fprintf(yyout, FORMAT_UJ, tok, sfx, sfx, msd);
			fprintf(yyout, FORMAT_UJV);
...
int print_ana()
{
		switch (multitoken) {
			case HOUR_DOT_MIN: // 10.32
				msd[2] = 'd';
				fprintf(yyout, FORMAT_ANAV, tok, msd);
